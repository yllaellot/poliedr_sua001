...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        # списки вершин, рёбер и граней полиэдра и нужного парметра из условия
        # (sua001)
    ...
    def _lenght(self):
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            # координаты центра ребра (sua001)
            p_center = (e.beg.__add__(e.fin)).__mul__(0.5)
            # проверка принадлежности центра ребра сфере радиуса 4 (sua001)
            if (len(e.gaps) == 0 and (p_center.x)**2 + (p_center.y)**2 +
                    (p_center.z)**2 <= 4):
                # учет длинны проекции (без координаты z считаем расстояние)
                # (sua001)
                lenght += ((e.beg.x - e.fin.x)**2 +
                           (e.beg.y - e.fin.y)**2)**(0.5)
        # Надо поделить на 2 поле lenght т.к. ребра повторяются ровно два раза
        # (sua001)
        lenght = (lenght)/2
        return (lenght)
